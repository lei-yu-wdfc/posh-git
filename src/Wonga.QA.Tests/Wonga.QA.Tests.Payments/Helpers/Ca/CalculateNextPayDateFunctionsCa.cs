...
namespace Wonga.QA.Tests.Payments.Helpers.Ca
{
    public class CalculateNextPayDateFunctionsCa
    {
        ...
        private static DateTime GetNextDayOfMonth15thOr30th(DateTime today)
        {
            ...
            var nextPayDate = new DateTime(payYear, payMonth, payDay);
            var stepBackDate = GetPreviousWorkingIfGivenPaydateIsNonWorkingDay(nextPayDate, nextPayDate);

            return stepBackDate < today ? GetNextDayOfWeek(nextPayDate, today.AddDays(1)) : stepBackDate;
        }
    }
}
