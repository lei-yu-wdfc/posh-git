...
namespace Wonga.QA.Tests.Risk.ZScoreTest
{
    class ZScoreTest
    {
        ...
        [Test, AUT(AUT.Wb)]
        [JIRA("SME-957"), Description("score card test with CallCredit"), Explicit("This test is disabling CallReport mock and as tests are being run in parallel, this makes other tests fail.")]
        [Ignore("We can't turn off mocks just only with db settings. No sense in this test")]
        public void LoadZScoreTestCustomers()
        {
            ...
            foreach (var customerdetail in customerdetails)
            {
                ...
                var Street = custdata[6];
                var City = custdata[7];
                CreateApplicationWithGuarantor(forename, surname, dateOfBirth, PostCode, HouseNumber, MiddleName, City, Street);
            }
        }
        ...
        private static void CreateApplicationWithGuarantor(string forename, string surname, Date dateOfBirth, string postCode, string houseNumber, string middleName, string city, string street)
        {
            ...
            var guarantor =
                guarantorBuilder.WithDateOfBirth(dateOfBirth).WithForename(forename).WithSurname(surname).
                    WithPostcodeInAddress(postCode).WithHouseNumberInAddress(houseNumber).
                    WithStreetInAddress(street).WithTownInAddress(city);
            
            var customer = CustomerBuilder.New().Build();
            var organization = OrganisationBuilder.New(customer).Build();
            var application = ApplicationBuilder.New(customer, organization) as BusinessApplicationBuilder;
