...
{
    static class CustomerCreationProgram
    {
        ...
        public static TimeSpan DayRun = new TimeSpan(10, 43, 00);
        ...
        {
            if (NumberOfUsersToCreate <= 0) return;
            var c = new VanillaCustomer();
            c.InsertUserToDb();
            Console.WriteLine(String.Format("User number {0} with email {1} successfully created by thread Id:{2}",
                                            thread.ToString(CultureInfo.InvariantCulture), c.Email,
