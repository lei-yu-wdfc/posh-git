...
//Step8:  Check the total record count in HDS which should be equal to 1 because hds is not capturing the record where _$Operation=3 if not through exception
...
{
    class CalendarDatesInsertAndUpdateSameLoadSameLsn
    {
        [Test]
        [AUT(AUT.Uk)]
        ...
        public void CalendarDatesInsert()
        {
            ...
            using (var transaction = Drive.Data.Payments.Db.BeginTransaction())
            {
                ...
                var calDtInsert = transaction.payment.CalendarDates.Insert(Date: newdate,
                                                                                     IsBankHoliday: 1,
                    ...
                    var calDtUpdate = transaction.payment.CalendarDates.UpdateByCalendarDateId(CalendarDateId: calDtInsert.CalendarDateId,
                                                                                                           IsBankHoliday: 0);
