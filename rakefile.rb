...
require 'fileutils'
require './run/ruby/gallio.rb'
require './run/ruby/msbuild_helper.rb'
...
task :config do #ready
  test_target = ENV['QAFTestTarget']
  FileUtils.cp CONFIG + '\\' + test_target + '.v3qaconfig', BIN unless test_target.empty?
end
  
task :pre_generate_serializers do #ready
  exec do |cmd|
    cmd.command = "LIB\\sgen\\sgen.exe"
    cmd.parameters = "BIN\\Wonga.QA.Framework.Cs.dll /force"
  end
end
#--
  
task :merge do #ready
  exclude = BIN + '\\' + TESTS + '.Core.dll'
  exclude += ' ' + BIN + '\\' + TESTS + '.Meta.dll'
  exclude += ' ' + BIN + '\\' + TESTS + '.Ui.dll'
  exclude += ' ' + BIN + '\\' + TESTS + '.Ui.Mobile.dll'
  exclude += ' ' + BIN + '\\' + TESTS + '.Migration.dll'
  ...
  command = PROGRAMM_FILES+'\\Microsoft\\ILMerge\\ILMerge.exe'
...
task :run_tests, :files, :filter do |t, fls, fltr| 
